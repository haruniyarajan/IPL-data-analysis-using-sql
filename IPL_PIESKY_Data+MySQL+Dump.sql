...
select a.BIDDER_ID,count(a.bid_status) as wins,((count(a.BID_STATUS)/sum(b.NO_OF_BIDS)) * 100) as win_percentage
...
select c.stadium_id,c.stadium_name,c.city,count(distinct(a.match_id)) as no_of_matches
 from ipl_match a join ipl_match_schedule b
 on a.match_id=b.match_id
 and b.status='completed'
 join ipl_stadium c
 on b.stadium_id=c.stadium_id
 group by c.stadium_id
 order by c.stadium_id;
...
select j.stadium_name,j.stadium_id,(ab/count(ms.stadium_id))*100 as '% win in a stadium when a team won the toss'
from j inner join ipl_match_schedule ms
on j.stadium_id=ms.stadium_id
group by j.stadium_name;

#4.	Show the total bids along with bid team and team name.
select a.bid_team as bid_team_id,c.team_name as bid_team,count(b.NO_OF_BIDS) as total_bids
...
#5.	Show the team id who won the match as per the win details.
 UPDATE IPL_MATCH            #Due to incorrect data, had to make changes.
 SET WIN_DETAILS='Team RR won by 35 Runs'
 WHERE MATCH_ID=1058;
 UPDATE IPL_MATCH
 SET WIN_DETAILS='Team MI won by 35 Runs'
 WHERE MATCH_ID=1056;
 ...
 end as WINNER_TEAM_ID,WIN_DETAILS
 from ipl_match
 GROUP BY MATCH_ID
 ORDER BY MATCH_WINNER;
 
 #6.	Display total matches played, total matches won and total matches lost by team along with
 #its team name.
 select a.TOURNMT_ID,a.team_id,b.team_name,a.matches_played,a.matches_won,a.matches_lost
 from ipl_team_standings a join ipl_team b
 on a.team_id=b.team_id
 order by a.tournmt_id;
 
 #7.	Display the bowlers for Mumbai Indians team.
 select c.player_name
from ipl_team a join ipl_team_players b
on a.team_id=b.team_id
and b.player_role='Bowler'
...
select a.team_id,a.team_name,count(c.player_id) as All_rounders_count
from ipl_team a join ipl_team_players b
on a.team_id=b.team_id
and b.player_role='All-Rounder'
join ipl_player c
on b.PLAYER_ID=c.PLAYER_ID 
group by a.team_id
